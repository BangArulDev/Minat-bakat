...
export default function KelasPage() {
  ...
  const [studentList, setStudentList] = useState([]); // Data siswa dummy
  ...
  // 4. Handle Detail Siswa
  const handleDetailClick = (item) => {
    setOpenDropdownId(null);
    setSelectedClass(item);
    
    // Generate Mock Students berdasarkan jumlah siswa di kelas
                        ...
                        <AnimatePresence>
                          {openDropdownId === item.id && (
                            <motion.div
                              ...
                              className="absolute right-8 top-8 w-40 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden text-left"
                            >
                              <button 
                                onClick={() => handleDetailClick(item)}
                                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-gray-600 hover:bg-purple-50 hover:text-purple-600 transition-colors"
                              >
                                <Eye size={16} /> Detail
                              </button>
          ...
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                ...
                <button onClick={() => setIsDetailOpen(false)} className="p-1 rounded-full hover:bg-gray-200 text-gray-500">
                  <X size={20} />
                </button>
              </div>
                      ...
                      studentList.map((siswa, idx) => (
                        <tr key={idx} className="hover:bg-gray-50">
                          ...
                          <td className="py-3 px-6 text-center">
                            <span className={`inline-block w-6 h-6 rounded-full text-xs font-bold leading-6 ${siswa.gender === 'L' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
                              {siswa.gender}
                            </span>
                          </td>
                        </tr>
          ...
          <div className="fixed inset-0 z-70 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-gray-500 mb-6 text-sm">
                Apakah Anda yakin ingin menghapus kelas <span className="font-bold text-gray-800">{itemToDelete?.name}</span>? Data siswa di dalamnya mungkin akan ikut terhapus.
              </p>
